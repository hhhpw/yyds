// https://juejin.cn/post/6969758357288648718#heading-14
...
class Socket {
  ...
  init(callback) {
    ...
    this.ws.onclose = Socket.errorDispatch("close");
    this.ws.onerror = Socket.errorDispatch("error");
  }
  ...
  emit(data, callback) {
    ...
    if (state === this.ws.OPEN) {
      ...
      // 连接中轮询
      Socket.eventPoll(state, this.ws.OPEN, 500, () => {
        this.ws.send(JSON.stringify(data));
        callback && callback();
        this.heartbeatDetect();
      });
